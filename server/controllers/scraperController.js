    ...
    addData(req, res) {
        ...
        for (let i = 0; i < req.body.artists.length; i++) {
            newartist = new Artist({
                ...
                "news": [{ "description": req.body.news.artists[i].description, "link": req.body.artists[i].news.link }],
            });
            ...
            for (let j = 0; j < req.body.artists[i].songs.length; j++) {
                newgenre = new Genre({
                    "genre_name": req.body.artists.songs[j].genre[0],
                });
                ...
                if (req.body.artists.songs[j].genre.length == 2) {
                    newgenre = new Genre({
                        "genre_name": req.body.artists.songs[j].genre[1],
                    });
                    result = newgenre.save()
                    if (result) {
